...
export function AdminPanel({ onBackToSite }: AdminPanelProps) {
  // Auth state - strictly false by default, requiring login
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(false);
  const [loginUser, setLoginUser] = useState("");
  const [loginPass, setLoginPass] = useState("");
  const [loginError, setLoginError] = useState("");

  // Tab state: "applicants" | "passwords" | "settings"
  const [activeTab, setActiveTab] = useState<"applicants" | "passwords" | "settings">("applicants");
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (enteredUser === (admin.username || "").toLowerCase() && enteredPass === admin.password) {
  ...
  const handleAddCredential = (e: React.FormEvent) => {
    ...
    const updated = addSavedPassword({
  ...
  const handleDeleteCredential = (id: string, name: string) => {
    if (window.confirm(`Delete password for "${name}"?`)) {
      const updated = deleteSavedPassword(id);
      setPasswords(updated);
        ...
        <div className="relative w-full max-w-md">
          ...
          <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-8 backdrop-blur-xl shadow-2xl shadow-black/80">
            <div className="flex flex-col items-center text-center">
              ...
              <h1 className="text-2xl font-black tracking-tight text-white">Aleph Admin Portal</h1>
              <p className="mt-1 text-xs text-slate-400">Sign in to manage applicants and site settings</p>
            </div>
